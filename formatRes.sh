...
# ヘッダーを出力ファイルに書き込む
echo "nmod,nver,Parsing,Compiling,Minimize,ConsRes_CPU,ConsRes_Real,ConsRes_sbv_elapsed,Extraction,set-option,declare-fun,assert,check-sat,get-value" > "$output"
...
while IFS= read -r line; do
    ...
    case $line in
        ...
        *"get-value"* ) get_value_count=$value
            # csvの行を出力ファイルに追加
            echo "$nmod,$nver,$parsing,$compiling,$minimize,$consres_cpu,$consres_real,$consres_sbv_elapsed,$extraction,$set_option_count,$declare_fun_count,$assert_count,$check_sat_count,$get_value_count" >> "$output"
            ;;
    esac
done < "$input"
