...
spec = do
  describe "Satisfiable" $ do
    it "a0 <= [a100]" $ do
      let cs = [ mkGtLabels "a0" [a100] ]
      res <- Data.Either.fromRight (error "") <$> solve em1 cs
      res `shouldBe` [("a0", [("A", v100), ("B", tbd)])]
    it "a0 <= [a100, a101]" $ do
      let cs = [ mkGtLabels "a0" [a100,a101] ]
      res <- Data.Either.fromRight (error "") <$> solve em1 cs
      res `shouldBe` [("a0", [("A", v101), ("B", tbd)])]
    it "a0 <= [a100], a1 <= [a100]" $ do
      let cs = [ mkGtLabels "a0" [a100], mkGtLabels "a1" [a100] ]
      res <- Data.Either.fromRight (error "") <$> solve em1 cs
      res `shouldBe`
        [ ("a0", [("A", v100), ("B", tbd)])
        , ("a1", [("A", v100), ("B", tbd)]) ]
    it "a0 <= a1, a1 <= [a100]" $ do
      let cs = [ mkGtVar "a0" "a1", mkGtLabels "a1" [a100] ]
      res <- Data.Either.fromRight (error "") <$> solve em1 cs
      res `shouldBe`
        [ ("a0", [("A", v100), ("B", tbd)])
        , ("a1", [("A", v100), ("B", tbd)]) ]
    it "a0 <= a1, a1 <= [a100, a101], a2 <= [a100, a101]" $ do
      let cs = [ mkGtVar "a0" "a1", mkGtLabels "a1" [a100, a101], mkGtLabels "a2" [a100, a101] ]
      res <- Data.Either.fromRight (error "") <$> solve em1 cs
      res `shouldBe`
        [ ("a0", [("A", v101), ("B", tbd)])
        , ("a1", [("A", v101), ("B", tbd)])
        , ("a2", [("A", v101), ("B", tbd)])]
    it "a0 <= a1, a1 <= [a100], a2 <= [a100, a101]" $ do
      let cs = [ mkGtVar "a0" "a1", mkGtLabels "a1" [a100], mkGtLabels "a2" [a100, a101] ]
      res <- Data.Either.fromRight (error "") <$> solve em1 cs
      res `shouldBe`
        [ ("a0", [("A", v100), ("B", tbd)])
        , ("a1", [("A", v100), ("B", tbd)])
